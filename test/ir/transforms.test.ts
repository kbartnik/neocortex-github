...
describe("IR Node Transformation", () => {
    it("should return Ok with transformed IRNode for valid GitHub Issue", () => {
        ...
        if (result.isOk()) {
            ...
            expect(node.data.body).toBe("When users try to log in with special characters...");
            ...
            expect(node.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
        }
    })
});